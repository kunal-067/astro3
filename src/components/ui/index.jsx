...
export function SectionBadge({ children }) {
  return (
    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-linear-to-r from-pink-500/20 to-purple-500/20 border border-pink-400/30 text-pink-400 text-xs font-semibold uppercase tracking-widest mb-5">
...
export function SectionHeading({ title, subtitle }) {
  return (
    <div className="text-center mb-6 md:mb-14">
      <h2 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-2 md:mb-4 leading-tight">
        {title}
      </h2>
...
export function Section({ children, className = "", id = "" }) {
  return (
    <section id={id} className={`py-10 mb:py-20 px-4 max-w-7xl mx-auto ${className}`}>
      {children}
    </section>
  );
}
...
export function Avatar({ name, size = "md" }) {
  ...
  return (
    <div className={`${sz} rounded-full bg-gradient-to-br from-[#ff4fa1] to-[#9333ea] flex items-center justify-center text-white font-bold flex-shrink-0`}>
      {name[0]}
    </div>
  );
}
